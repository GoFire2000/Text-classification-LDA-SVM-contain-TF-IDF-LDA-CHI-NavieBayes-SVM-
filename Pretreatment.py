...
def FenciProcess(initial_path, target_path):
    ...
    for mydir in dir_list:
        ...
        for myfile in file_list:
            ...
            # 调用jieba，进行分词
            content_fenci = list(jieba.cut(content))
            words = pseg.cut(content)
            
            # print(content_fenci)
            newContent = []
            for myword, myflag in words:
                if myword in stop_words_dict:
                    continue
                if ('n' in myflag) or ('v' in myflag) or ('a' in myflag): #  根据词性，保留名词动词形容词
                    newContent.append(myword + '/' + myflag)
                # print(newContent[-1])
...
if __name__ == "__main__":
    ...
    target_sets_path = "./ProcessedTrainingSetsLabeled/" # 分词后训练集路径
    ...
    target_sets_path = "./ProcessedTestSetsLabeled/"
    FenciProcess(initial_sets_path, target_sets_path)
